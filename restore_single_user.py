...
def run_script(dbname, script):
    ...
    p1 = subprocess.Popen('psql -v ON_ERROR_STOP=1 -p %d -t %s -U %s %s < %s' %
                          (c.DB_PORT, db_address_with_flag, c.DB_USERNAME, dbname, scriptfile[1]),
                          shell=True,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Restore DBs")
    parser.add_argument('--reset-versions', action='store_true', default=False)
    parser.add_argument('--reset-sync', action='store_true', default=False)
    ...
    reset_versions = args.reset_versions
    ...
    dumps_to_restore = []  # FIXME: Add logic to select only a few dumps
    ...
    try:
        ...
        for filename in os.listdir(environment_dir):
            ...
            if args.prefix:
                dbname = utils.prefix_db_name(dbname, prefix=args.prefix)
            else:
        ...
        for dbname, filename in to_restore:
            ...
            p1 = subprocess.Popen('pg_restore -p %d %s -U %s --no-acl --no-owner -n public -d %s %s' %
                                  (c.DB_PORT, db_address_with_flag, c.DB_USERNAME, dbname, path_dump),
                                  shell=True,
                                  stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE)
            ret, stderr = p1.communicate()
            ...
            run_script(dbname, "UPDATE res_users SET password = '%s' WHERE login = '%s'" %
                       (c.UNIFIELD_PASSWORD, c.UNIFIELD_ADMIN))
            ...
            if filter(lambda x : x, ret.split('\n')):
                ...
                if reset_versions:
                    run_script(dbname,
                               ...
                               "WHERE sum NOT IN ('88888888888888888888888888888888', "
                               "'66f490e4359128c556be7ea2d152e03b')")
            else:
                ...
                run_script(dbname,
                           "UPDATE sync_client_sync_server_connection SET database = '%s', "
                           "host = 'localhost', protocol = 'xmlrpc', port = %d" % (sync_server_db, c.XMLRPC_PORT))
                if reset_versions:
                    run_script(dbname,
                ...
                if reset_sync:
                    run_script(dbname,
                               "UPDATE ir_model_data SET create_date = '2016-05-24' "
                               "WHERE create_date > '2016-05-24' OR create_date IS NULL")
                    run_script(dbname,
                               "UPDATE ir_model_data SET last_modification =  "
                               "CASE WHEN sync_date < last_modification OR sync_date IS NULL "
                               "THEN TIMESTAMP '2016-05-24 23:00:00' "
                               "ELSE TIMESTAMP '2016-05-24 01:00:00' END")
